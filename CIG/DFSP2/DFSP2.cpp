            ...
            for (int Ins = 0; Ins < 90; Ins++)
            {
              ...
              /*  ofile << "DIG" << "\n";*/
                ...
                for (int i = 0; i < 20; i++)        //每个测试用例执行20遍
                {
                    ...
                    SpanFactory_EA[Ins][i] = EA(TimeLimit, Psize);   //不同的算法
              ...
              /*  ofile << "IG" << "\n";*/
               ...
               /* ofile << "DPSO" << "\n";*/
                ...
                fprintf(f1, "%lf", RPI[0]); fprintf(f1, "\t");
                RPI[1] = (min_DDE - min_span) / min_span * 100;
                fprintf(f1, "%lf", RPI[1]); fprintf(f1, "\t");
                RPI[2] = (min_EA - min_span) / min_span * 100;
                fprintf(f1, "%lf", RPI[2]); fprintf(f1, "\t");
                RPI[3] = (min_MN_IG - min_span) / min_span * 100;
                fprintf(f1, "%lf", RPI[3]); fprintf(f1, "\t");
                RPI[4] = (min_IG - min_span) / min_span * 100;
                fprintf(f1, "%lf", RPI[4]); fprintf(f1, "\t");
                RPI[5] = (min_PSO - min_span) / min_span * 100;
                fprintf(f1, "%lf", RPI[5]); fprintf(f1, "\t");
            
                fprintf(f1, "\n");
                
                cout <<  RPI[0] << "\t"  << RPI[1] << "\t"  << RPI[2] << "\t"  << RPI[3] << "\t"  << RPI[4] << "\t"  << RPI[5] << endl;
                cout << endl;
                ofile << endl;
